...
- `seen` **只存雜湊、不存原始 IP／User-Agent、不可逆推**；前天以前的列由**每日 Cron**（`wrangler.toml` 的 `[triggers] crons`，20:00 UTC＝台北隔日 04:00）清掉，不再佔用計數熱路徑。
- `/hit` **只接受帶合法 `Origin` 的瀏覽器請求**（白名單見 `ALLOWED_ORIGINS`），擋掉裸 `curl`／腳本灌水；真訪客跨網域一定帶 Origin，不受影響。
