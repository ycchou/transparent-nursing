 ...
 *
 *   node tools/visual-snapshot.mjs before index,stats   # 只截指定頁
 *
...
const OUT = path.join(ROOT, '.build-cache', 'visual', label);
const ALL_PAGES = fs.readdirSync(ROOT)
  .filter((f) => f.endsWith('.html') && !/^platform-full-/.test(f))
  .map((f) => f.replace(/\.html$/, ''))
  .sort();
const PAGES = process.argv[3] ? process.argv[3].split(',') : ALL_PAGES;
const VIEWPORTS = [
...
const profile = fs.mkdtempSync(path.join(ROOT, '.build-cache', 'chrome-'));
...
try {
  for (const vp of VIEWPORTS) {
    ...
    for (const page of PAGES) {
      inflight.clear();
      await send('Page.navigate', { url: `${BASE}/${page}.html?data=mock` });
      await waitQuiet();
      ...
      fs.writeFileSync(path.join(OUT, `${page}@${vp.name}.png`), Buffer.from(shot.data, 'base64'));
      n++;
      process.stdout.write(`\r截圖 ${n}/${PAGES.length * VIEWPORTS.length}  ${page}@${vp.name}        `);
    }
  }
  console.log(`\n✔ 已存到 ${path.relative(ROOT, OUT)}/`);
} finally {
  cleanup();
}
process.exit(0);
