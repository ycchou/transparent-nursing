// Inline SVG icon library (subset of Lucide)
// Usage: icon('activity', { size: 24, className: 'text-primary' })
...
const ICONS = {
  ...
  // 站標同款心型（中央凹陷較深），供捐款/支持處使用；預設以 CSS 設 fill 呈實心
  'heart-full': '<path d="M12 20.6 c-5.6 -3.8 -9.2 -8.2 -9.2 -13.0 a4.6 4.6 0 0 1 9.2 -1 a4.6 4.6 0 0 1 9.2 1 c0 4.8 -3.6 9.2 -9.2 13.0 z"/>',
...
export function icon(name, opts = {}) {
  const { size = 20, className = '', stroke = 'currentColor' } = opts;
...
/** Replace all <i data-icon="name"></i> placeholders with inline SVG */
export function renderIcons(root = document) {
  root.querySelectorAll('[data-icon]').forEach((el) => {
    const name = el.dataset.icon;
    const size = el.dataset.size || 20;
    const cls = el.className || '';
    el.outerHTML = icon(name, { size: +size, className: cls });
  });
}
