...
前端 →（帶 Turnstile token）→ 本 Worker → ①驗 Turnstile ②內容檢查
...
防護對應本專案「反垃圾」分層的 ①②。其中：
- **③ honeypot + 填寫耗時（≥1 分鐘）** 已在前端 `js/form-engine.js`。
- **每日上限 5 筆、兩次間隔 ≥5 分鐘** 也在前端（單一裝置 localStorage）→ 本 Worker 不做 IP 限流、不需 D1。
...
- Worker（`src/index.js` 頂部）：`MAX_LINKS`（自由文字允許連結數，預設 0）、`ALLOWED_ORIGINS`。
